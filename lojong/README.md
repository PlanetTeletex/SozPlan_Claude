...
- **Contemplation timer** — its own view, showing which slogan you are sitting
  with, and an optional breath pacer — four seconds in, six out, after slogan 7,
  *these two should ride the breath*; the words keep pacing even where motion is
  switched off. 3 / 5 / 10 minutes, with a synthesised singing-bowl tone
  generated by the Web Audio API (no audio file). The bell can be switched off; the
  choice is remembered in `localStorage`.
- **The controls ring the dial** — seven marks on a circle around the timer, set
  apart from each other and from the dial: the three lengths across the top, the
  bell and the breath pacer at the sides, start at the bottom and reset beside it.
  Each carries a number or a drawn glyph with an `aria-label`, and a quiet line
  under the ring names what the two wordless marks are doing.
